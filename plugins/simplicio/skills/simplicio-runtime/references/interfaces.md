...
The Runtime decision receipt contains peer fields for `mapper`, `fast`, and
`loop`. Dev CLI is nested at `loop.dev_cli` only when Loop is active. A direct
file mutation uses the `simplicio edit` mechanical writer by default. Standalone
operator calls remain valid without a Runtime transaction or cross-component flow;
missing optional evidence is reported as degraded or `UNVERIFIED`.
