...
describe("read-only snapshot boundary", () => {
  ...
  it("does not time out or replay login, logout or installation", async () => {
    ...
    void bridge.repairDesktopProviders(`sha256:${"a".repeat(64)}`).then(outcome, outcome);
    await vi.advanceTimersByTimeAsync(120_000);
    expect(outcome).not.toHaveBeenCalled();
    expect(invokeMock.mock.calls.map(([command]) => command)).toEqual(["desktop_login", "desktop_logout", "desktop_repair_providers"]);
