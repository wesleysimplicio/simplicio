  ...
  return (
    <div className="page product-page">
      ...
      <div className="chat-layout">
        <aside className="panel session-list"><div className="panel-heading"><div><span className="eyebrow">Sessões</span><h2>Chats</h2></div><span className="queue-count">{workspace.sessions.length}</span></div>{workspace.sessions.map((chat) => <button className={`session-card ${workspace.selectedSessionId === chat.sessionId ? "active" : ""}`} type="button" key={chat.sessionId} disabled title="Troca de sessão aguardando action descriptor do Runtime"><span className="bot-avatar">{chat.title.slice(0, 1).toUpperCase()}</span><span><strong>{chat.title}</strong><small>{chat.state} · rev. {chat.revision} · {chat.unread} eventos</small></span></button>)}<ActionButton title="Criar sessão exige o Agent API do Runtime">+ Novo chat</ActionButton></aside>
