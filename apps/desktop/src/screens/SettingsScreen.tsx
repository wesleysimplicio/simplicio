...
export function SettingsScreen({ snapshot, busy, onRefresh, onSubscribe, onLogout, logoutBusy, section = "all", preferences = DEFAULT_PREFERENCES, onPreferences = () => undefined, onUninstall }:
  ...
  return <div className="page preferences-page account-page">
    ...
    {section !== "diagnostics" && <section className="settings-section"><h2>Preferências do Desktop</h2><div className="settings-slab">
      ...
      <div className="preference-row"><div><strong>Encerrar sessões ociosas</strong><p>Após 15 minutos sem atividade da sessão, o Runtime fecha só a sessão lógica e consolida tokens. Pedidos em andamento não contam como ociosos. Processos de Claude, Codex e outros hosts não são encerrados.</p></div><label className="preference-toggle"><input type="checkbox" checked={preferences.closeIdleSessions} onChange={(event) => updatePreferences({ closeIdleSessions: event.target.checked })} aria-label="Encerrar sessões ociosas após 15 minutos" />{preferences.closeIdleSessions ? "Ativado" : "Desativado"}</label></div>
